...
// QueryResolver builder generates Resolve section for Go.
...
func (b *QueryResolver) BuildSchema() (string, error) {
	return "", nil
}

func (b *QueryResolver) BuildProgram() (string, error) {
	...
	var response string
	if b.m.ExposeQuery() != "" {
		field := b.m.ExposeQueryFields(output)[0]
...
func (b *QueryResolver) buildRequiredParams(f *spec.Field) (string, error) {
	t := f.GoType()
	name := f.Name()
...
func (b *QueryResolver) buildOptionalParams(f *spec.Field) (string, error) {
	t := f.GoType()
	name := f.Name()
	camelName := strcase.ToCamel(name)
